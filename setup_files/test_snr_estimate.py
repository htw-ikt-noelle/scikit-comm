...
# SNR = 10 #in dB 
SNR = np.arange(0,31,dtype='float') 
...
for i in range(len(SNR)): 
    ...
    for j in range(MC_runs): 
        ...
        # generate bits 
        sig_tx.generate_bits(n_bits=int(np.log2(mod_order))*(2**8)) 
        ...
        # calc EVM 
        evm = comm.rx.calc_evm(sig_tx, norm='rms', method='blind', dimension=-1) 
        evm_vec[i,j] = evm 
        
        evm_ref = comm.rx.calc_evm(sig_tx, norm='rms', method='symbol_aided', dimension=-1) 
