...
if SINC_CORRECTION:
    for dim in range(n_dims):
        sig_tx.samples[dim] = comm.pre_distortion.dac_sinc_correction(sig_tx.samples[dim],
                                                                      f_max=1.0)
...
if USE_PREDIST:
    ...
    for dim in range(n_dims):
        sig_tx.samples[dim] = comm.filters.filter_arbitrary(sig_tx.samples[dim], 
                                                            filtershape, 
                                                            sample_rate=sig_tx.symbol_rate[dim]*TX_UPSAMPLE_FACTOR)

# TODO: equalization of cosine MZM transfer function

# format samples so that driver can handle them (range +-1)
# note: only first dimension of signal object is being used, up to this point,
...
#%% # Simulation 
    ...
    # write identical sample vector into every dimension of signal object
    sig_tx.samples = samples
    ...
    #%% ## add amplitude noise
    for dim in range(n_dims):
        sig_tx.samples[dim] = comm.channel.set_snr(sig_tx.samples[dim], snr_dB=SNR[dim], sps=sig_tx.sample_rate[dim]/sig_tx.symbol_rate[dim], seed=None)

    ##%% ## phase noise emulation
    # TODO: call function with unique phase noise seeds per dimension, if different
    # LOs are being used 
    for dim in range(n_dims):
        sig_tx.samples[dim] = comm.channel.add_phase_noise(sig_tx.samples[dim] ,sig_tx.sample_rate[dim] , LASER_LINEWIDTH, seed=1)['samples']
    ...
    # samples = f(t_new)    
    sig_tx = comm.utils.add_sampling_error(sig_tx, ratio=1.0)
